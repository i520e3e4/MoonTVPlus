...
const Logo = () => {
  ...
  return (
    <Link
      href='/'
      className='netflix-wordmark flex items-center justify-center h-16 select-none hover:opacity-80 transition-opacity duration-200'
    >
      <span className='text-2xl font-black text-red-600 tracking-[-0.04em]'>
        {siteName}
      </span>
    </Link>
  );
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      ...
      <div className='hidden md:flex'>
        <aside
          data-sidebar
          className={`netflix-desktop-nav fixed top-0 left-0 h-screen bg-white/40 backdrop-blur-xl transition-all duration-300 border-r border-gray-200/50 z-10 shadow-lg dark:bg-gray-900/70 dark:border-gray-700/50 ${
        ...
        >
          <div className='netflix-nav-inner flex h-full flex-col'>
            {/* 顶部 Logo 区域 */}
            <div className='netflix-logo-zone relative h-16'>
              <div
            ...
            {/* 首页和搜索导航 */}
            <nav className='netflix-primary-nav px-2 mt-4 space-y-1'>
              <Link
                href='/'
                prefetch={false}
                onClick={(e) => {
            ...
            {/* 菜单项 */}
            <div className='netflix-category-nav flex-1 overflow-y-auto px-2 pt-4'>
        ...
        <div
          className={`netflix-sidebar-offset transition-all duration-300 sidebar-offset ${
            isCollapsed ? 'w-16' : 'w-64'
          }`}
        ></div>
      </div>
